...
if [ $embed_num -eq $wavs_num ]; then
  echo "emb: Successfully extract embedding for ${store_dir}" | tee ${embed_dir}/extract.result
else
  echo "emb: Failed to extract embedding for ${store_dir}" | tee ${embed_dir}/extract.result
fi
if [ $variance_num -eq $wavs_num ]; then
  echo "var: Successfully extract embedding for ${store_dir}" | tee ${embed_dir}/extract.result
else
  echo "var: Failed to extract embedding for ${store_dir}" | tee ${embed_dir}/extract.result
fi