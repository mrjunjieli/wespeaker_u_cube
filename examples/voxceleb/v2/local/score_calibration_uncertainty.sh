...
if [ $stage -le 1 ] && [ $stop_stage -ge 1 ]; then
  ...
  python wespeaker/bin/score_norm_forvox2.py \
    --score_norm_method $score_norm_method \
    --top_n $top_n \
    --trial_score_file $exp_dir/scores/${calibration_trial}.score \
    --score_norm_file $exp_dir/scores_uncertainty/${output_name}_${calibration_trial}.score \
    ...
    --eval_uncertainty_scp ${exp_dir}/embeddings/vox2_dev/xvector_variance.scp \
...
if [ $stage -le 2 ] && [ $stop_stage -ge 2 ]; then
  ...
  mkdir -p ${exp_dir}/scores_uncertainty/calibration
  cat ${data}/vox1/utt2dur ${data}/vox2_dev/utt2dur > ${exp_dir}/scores_uncertainty/calibration/utt2dur
  for x in ${calibration_trial} $trials; do
    python wespeaker/bin/score_calibration_uncertain.py "gather_calibration_factors" \
      --wav_dur_scp ${exp_dir}/scores_uncertainty/calibration/utt2dur \
      --max_dur 20 \
      --score_norm_file ${exp_dir}/scores_uncertainty/${output_name}_${x}.score \
      --calibration_factor_file ${exp_dir}/scores_uncertainty/calibration/${output_name}_${x}.uncertainty_calibration
  done
fi
...
if [ $stage -le 3 ] && [ $stop_stage -ge 3 ]; then
  ...
  python wespeaker/bin/score_calibration_uncertain.py "train_calibration_model" \
    --calibration_factor_file ${exp_dir}/scores_uncertainty/calibration/${output_name}_${calibration_trial}.uncertainty_calibration \
    --save_model_path ${exp_dir}/scores_uncertainty/calibration/calibration_model.pt
fi
...
if [ $stage -le 4 ] && [ $stop_stage -ge 4 ]; then
  ...
  for x in ${trials}; do
    python wespeaker/bin/score_calibration_uncertain.py "infer_calibration" \
      --calibration_factor_file ${exp_dir}/scores_uncertainty/calibration/${output_name}_${x}.uncertainty_calibration \
      --save_model_path ${exp_dir}/scores_uncertainty/calibration/calibration_model.pt \
      --calibration_score_file ${exp_dir}/scores_uncertainty/${cali_output_name}_${x}.score
  done
fi
