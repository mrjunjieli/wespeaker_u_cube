...
from wespeaker.models.spkd_loss import SPKDLoss
    ...
    for i, batch in enumerate(dataloader):
        ...
        with torch.cuda.amp.autocast(enabled=configs['enable_amp']):
            ...
            if not isinstance(outputs, tuple) or len(outputs) != 5:
                raise ValueError(
                    "RecXi training expects (frame_feat, variance, embedding, "
                    "teacher, student)")
            ...
            teacher = outputs[3]
            student = outputs[4]
            proj_out = model.module.projection(embeds, variance, targets)
            if isinstance(proj_out, tuple):
                outputs, class_loss = proj_out
            else:
                outputs = proj_out
                class_loss = criterion(outputs, targets)

            kd_loss = SPKDLoss()(teacher, student)
            classifi_ratio = configs.get('classifi_ratio', 1.0)
            kd_ratio = configs.get('kd_ratio', 3000.0)
            loss = classifi_ratio * class_loss + kd_ratio * kd_loss
